...
Drupal.jsACLink = function ($input, db) {
  ...
  object.select = function (node) {
    ...
    $('#' + this.input.id.substr(0, this.input.id.length - 6) + '-url').val('node/' + $(node).data('autocompleteNode'));
  };
  ...
  object.hidePopup = function (keycode) {
    ...
    if (this.selected && ((keycode && keycode != 46 && keycode != 8 && keycode != 27) || !keycode)) {
      ...
      $('#' + this.input.id.substr(0, this.input.id.length - 6) + '-url').val('node/' + $(this.selected).data('autocompleteNode'));
    }
    // Hide popup.
